...
test.describe("POST /auth/login", () => {
  ...
  test("Usuario no existe", async ({ request }) => {
    const response = await request.post("https://dummyjson.com/auth/login", {
      data: { username: "usuariofantasma99", password: "cualquiera" },
      headers: { "Content-Type": "application/json" }
    });
    ...
    const body = await response.json();
    expect(body.message).toBeTruthy(); // Verificar que contenga un mensaje de error
  });
  ...
  test("Contraseña vacía / no enviada", async ({ request }) => {
    const response = await request.post("https://dummyjson.com/auth/login", {
      data: { username: "emilys" }, // Sin campo password
      headers: { "Content-Type": "application/json" }
    });
  ...
  test("Contraseña incorrecta para usuario existente", async ({ request }) => {
    const response = await request.post("https://dummyjson.com/auth/login", {
      data: { username: "emilys", password: "passwordincorrecto123" },
      headers: { "Content-Type": "application/json" }
    });
